...
namespace DatabaseAccess
{
    public sealed class DbAccess : IDisposable
    {
        ...
        public DbAccess(string connName)
        {
            //obter o nome da máquina (servidor)
            //Data Source=NCC1701D-MS;

            string nome = Environment.MachineName;
            string nomeServidor = Dns.GetHostEntry(nome).HostName;

            string connString = ConfigurationManager.ConnectionStrings[connName].ConnectionString;
            string connectionString = string.Format("Data Source={0};" + connString, nomeServidor);
